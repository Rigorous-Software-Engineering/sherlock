 ...
 * In while(1) loop, two indicies are selected non-deterministically, whose
 entries in array are zero.
 ...
 * Variable count is reset on reaching 200. One can have any other integer value
 as well
 * to reset count. It is an additional challenge for Tools/solvers using
 supervised learning
 * based techniques for algorithm selection. 200 is the max unrolling for most
 of the reachsafe
 * programs to reach property violation. Presence of 200 as part of statement
 assignment
 * inside loop can be picked up by algorithm selector to deploy only
 falsification/correctness proving
 * techniques.
...
void reach_error() {
  __assert_fail("0", "array4_pattern.c", 35, "reach_error");
}
extern void abort(void);
