 ...
 * Array[index1][index2] is initialized with num^4, if it is antidiagonal
 * element. Sum of array should never exceed rowsize/columnsize.
 * */
...
void reach_error() {
  __assert_fail("0", "array14_pattern.c", 27, "reach_error");
}
extern void abort(void);
...
int main() {
  ...
  while (1) {
    ...
    array[index1][index2] = (index1 + index2 == ARR_SIZE - 1)
                                ? (num * num * num * num)
                                : array[index1][index2];
