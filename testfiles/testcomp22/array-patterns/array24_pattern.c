...
void reach_error() {
  __assert_fail("0", "array24_pattern.c", 28, "reach_error");
}
extern void abort(void);
...
int main() {
  ...
  while (1) {
    ...
    array[index1][index2] =
        (outerBoundaryPatternCheck(index1) || outerBoundaryPatternCheck(index2))
            ? array[index1][index2]
            : (num * num);
  ...
  __VERIFIER_assert(sum >= 0 && sum <= (ARR_SIZE - 2) * (ARR_SIZE - 2));
  return 0;
}
