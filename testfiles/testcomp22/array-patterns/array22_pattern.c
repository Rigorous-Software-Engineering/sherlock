...
void reach_error() {
  __assert_fail("0", "array22_pattern.c", 28, "reach_error");
}
extern void abort(void);
...
int main() {
  ...
  while (1) {
    ...
    array[index1][index2] = (outerBoundaryPatternCheck(index2))
                                ? (num * num)
                                : array[index1][index2];
    array[index1][ARR_SIZE - 1 - index2] =
        (outerBoundaryPatternCheck(index2))
            ? num
            : array[index1][ARR_SIZE - 1 - index2];
