...
void reach_error() {
  __assert_fail("0", "array23_pattern.c", 28, "reach_error");
}
extern void abort(void);
...
int main() {
  ...
  while (1) {
    ...
    array[index1][index2] = (outerBoundaryPatternCheck(index1))
                                ? (num * num)
                                : array[index1][index2];
    array[ARR_SIZE - 1 - index1][index2] =
        (outerBoundaryPatternCheck(index1))
            ? num
            : array[ARR_SIZE - 1 - index1][index2];
