...
void __CS_cs(void)
{
        ...
        if (__CS_SwitchPoints[__CS_SwitchDone] == __CS_StmtCount++) {
                ...
                if(!(__CS_round+k < __CS_ROUNDS)) {abort();}   // k==__CS_round
--> no switch
                __CS_round += k;
                //if(!(__CS_thread_lockedon[__CS_round][__CS_thread_index] ==
0)) {abort();}
...
int __CS_pthread_join(__CS_pthread_t thread, void **value_ptr) {
  ...
  __THREAD_FINISHED_NO_ERROR )) {abort();}
  */
...
int main() {
  ...
  if (!(__CS_thread_status[0][0] == __CS_cp___CS_thread_status[1][0])) {
    abort();
  } // cseq: Consistency of __CS_thread_status
  ...
  if (!(__CS_thread_lockedon[0][0] == __CS_cp___CS_thread_lockedon[1][0])) {
    abort();
  } // cseq: Consistency of __CS_thread_lockedon
  ...
  if (!(ma[0] == __CS_cp_ma[1])) {
    abort();
  } // cseq: Consistency of ma
  if (!(mb[0] == __CS_cp_mb[1])) {
    abort();
  } // cseq: Consistency of mb
  if (!(data1[0] == __CS_cp_data1[1])) {
    abort();
  } // cseq: Consistency of data1
  if (!(data2[0] == __CS_cp_data2[1])) {
    abort();
  } // cseq: Consistency of data2
