...
void __CS_cs(void) {
  ...
  __CS_round += k;
  assume_abort_if_not(__CS_thread_lockedon[k][__CS_thread_index] ==
                      0); // this is removed when not needed
...
int main() {
  ...
  assume_abort_if_not(num[0] == __CS_cp_num[1]); // cseq: Consistency of num
  assume_abort_if_not(m[0] == __CS_cp_m[1]);     // cseq: Consistency of m
  assume_abort_if_not(empty[0] ==
                      __CS_cp_empty[1]);           // cseq: Consistency of empty
  assume_abort_if_not(full[0] == __CS_cp_full[1]); // cseq: Consistency of full
