...
extern int getopt_long(int ___argc, char *const *___argv,
                       ...
                       int *__longind);
extern int smp_initiator_open(char const *device_name, int subvalue,
                              char const *i_params, unsigned long long sa,
                              struct smp_target_obj *tobj, int verbose);
extern int smp_send_req(struct smp_target_obj const *tobj,
...
static int do_discover_list(struct smp_target_obj *top, int sphy_id,
...
struct smp_func_def_rrlen smp_def_rrlen_arr[32] = {
    {0x0, 0, 6},    {0x1, 0, 14},   {0x2, -3, -3},  {0x3, -2, -2},
    {0x4, -2, -2},  {0x5, -2, -2},  {0x6, -2, -2},  {0x7, -3, -3},
    {0x10, 2, 0xc}, {0x11, 2, 6},   {0x12, 2, 13},  {0x13, 2, 9},
    {0x14, -2, -2}, {0x20, -2, -2}, {0x21, -2, -2}, {0x22, -2, -2},
    {0x80, 3, 0},   {0x81, -2, 0},  {0x82, -3, -3}, {0x83, -3, -3},
    {0x85, -2, 0},  {0x86, -2, -2}, {0x87, -2, 0},  {0x88, -2, 0},
    {0x89, -2, 0},  {0x8a, -2, 0},  {0x8b, -2, 0},  {0x90, 9, 0},
    {0x91, 9, 0},   {0x92, 9, 0},   {0x93, -2, 0},  {-1, -1, -1}};
...
void dStrHex(char const *str, int len, int no_ascii) {
  ...
  ((("1_1_247_8589935493" /*Grp 1*/,
     __VERIFIER_assert(k + 1 >= 0 && k + 1 < 82))),
   buff[k + 1] = ' ');
  for (i = 0; i < len; i++) {
    c = *p++;
    bpos += 3;
    ...
    ((("1_1_254_8589935534" /*Grp 1*/,
       __VERIFIER_assert(bpos >= 0 && bpos < 82)),
      (__tmpTR__int_1 = sprintf(&buff[bpos], "%.2x", (int)(unsigned char)c))),
     __tmpTR__int_1);
    ((("1_1_255_8589935545" /*Grp 1*/,
       __VERIFIER_assert(bpos + 2 >= 0 && bpos + 2 < 82))),
     buff[bpos + 2] = ' ');
    if (no_ascii) {
      ((__tmpTR__int_1 = cpos, cpos = __tmpTR__int_1 + 1,
        ("1_1_257_8589935554" /*Grp 1*/,
         __VERIFIER_assert(__tmpTR__int_1 >= 0 && __tmpTR__int_1 < 82))),
       buff[__tmpTR__int_1] = ' ');
    } else {
      if ((c < ' ') || (c >= 0x7f)) {
        c = '.';
...
static int do_discover_list(struct smp_target_obj *top, int sphy_id,
                            unsigned char *resp, int max_resp_len,
                            struct opts_t *op) {
  unsigned char smp_req[32] = {0x40, 0x20, 0, 6, 0, 0, 0, 0, 0, 0, 0,
...
int main() {
  ...
  int ret = do_discover_list(&tobj, __VERIFIER_nondet_int(), resp, sizeof(resp),
                             &opts);
  return 0;
}
