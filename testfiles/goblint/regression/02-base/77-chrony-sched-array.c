// PARAM: --set ana.malloc.wrappers '["Malloc", "Realloc", "Malloc2",
// "Realloc2", "ARR_CreateInstance", "realloc_array", "ARR_GetNewElement"]'
// --disable sem.unknown_function.spawn --disable
// sem.unknown_function.invalidate.globals extracted from chrony
#include <stdlib.h>
...
unsigned int ARR_GetSize(ARR_Instance array) { return array->used; }
...
void SCH_AddFileHandler(SCH_FileHandler handler, SCH_ArbitraryArgument arg) {
  int fd; // rand
...
static void dispatch_filehandlers() {
  FileHandlerEntry *ptr;
  int fd; // rand
