...
int main() {
  ...
  // acos(x):
  __goblint_check((0. <= acos(0.1)) &&
                  (acos(0.1) <= greater_than_pi)); // SUCCESS
  __goblint_check(acos(1.) == 0.);                 // SUCCESS
  acos(2.0); // WARN: Domain error might occur: acos argument might be outside
             // of [-1., 1.]

  // asin(x):
  __goblint_check(((-greater_than_pi / 2.) <= asin(0.1)) &&
                  (asin(0.1) <= (greater_than_pi / 2.))); // SUCCESS
  __goblint_check(asin(0.) == 0.);                        // SUCCESS
  asin(2.0); // WARN: Domain error might occur: asin argument might be outside
             // of [-1., 1.]

  // atan(x):
  __goblint_check(((-greater_than_pi / 2.) <= atan(0.1)) &&
                  (atan(0.1) <= (greater_than_pi / 2.))); // SUCCESS
  __goblint_check(atan(0.) == 0.);                        // SUCCESS

  // atan2(y, x)
  __goblint_check(((-greater_than_pi / 2.) <= atan2(0.1, 0.2)) &&
                  (atan2(0.1, 0.2) <= (greater_than_pi / 2.))); // SUCCESS
