...
int main() {
  ...
  if (a + high_number <= high_number) {
    // the mathematical solution with `a <= 0` is obviously not the only one
    // here
    __goblint_check(a <= 0.); // UNKNOWN!
    // assuming the rounding mode "nearest", `a` had to be below
    // `distance_to_next / 2` (as half-way cases are always rounded away from
    // zero)
    __goblint_check(a < (distance_to_next / 2)); // UNKNOWN!
    // ... this than also has to be unknown
    __goblint_check(a <= (distance_to_next / 2)); // UNKNOWN!
    // with up/down as rounding mode -> a only has to be below
    // `distance_to_next`
  ...
  double one = 1.;
  if (a * one >= next) {
    // the mathematical solution again would be `a >= next` but does not take
    // the rounding mode into account
    __goblint_check(a >= next); // UNKNOWN!
    // assuming the rounding mode "nearest", `a` had to be at least `high_number
    // + distance_to_next / 2` (as half-way cases are always rounded away from
    // zero)
