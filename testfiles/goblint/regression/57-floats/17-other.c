...
int main() {
  ...
  // On OS X this gets expanded differently than on Linux where it is equivalent
  // to the one below Might make sense to check what is needed for OS X support
  // in the future, but this is not a deal-breaker and not high priority for
  // now.
  ...
  __goblint_check((signbit(1.0 * -0.0))); // TODO (we don't distinguish 0 and
                                          // -0)
  __goblint_check((!signbit(1.0 * 0.0))); // TODO (we don't distinguish 0 and
                                          // -0)
  ...
  int zero = 0;
  __goblint_check(
      (isinf(INFINITY / zero))); // TODO (we don't distinguish 0 and -0)
  __goblint_check((0.0 / INFINITY == 0));
  __goblint_check((1.0 / INFINITY == 0));
  __goblint_check(
      (signbit(-1.0 / INFINITY))); // TODO (we don't distinguish 0 and -0)
  __goblint_check(
      (signbit(1.0 / -INFINITY))); // TODO (we don't distinguish 0 and -0)
