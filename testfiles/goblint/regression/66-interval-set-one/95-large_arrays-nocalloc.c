...
// Test to check whether partitioned arrays can have an index expression
// evaluating to values largers than the max value of int64

#define LENGTH (LONG_MAX - 600)
#define STOP (LENGTH - 1)

int main() {
  // Check that ptrdiff_t is at least as big as long, so we can index arrays
  // with non-negative longs
