// PARAM: --enable ana.int.interval_set --set sem.int.signed_overflow
// assume_wraparound With sem.int.signed_overflow set to assume_wraparound to
// assume two's complement representation for signed ints and don't go to top on
...
int main() {
  ...
  if (y / x == 2 && x == 3) {
    __goblint_check(x == 3); // TO-DO y == [6,8]; this does not work because CIL
                             // transforms this into two if-statements
  }
...
int main2() {
  ...
  if (x > one && x < five && x % two == one) {
    __goblint_check(x !=
                    two); // [two,four] -> [three,four] TO-DO x % two == one
  }
  ...
  if (ys / xs == two && xs == three) {
    __goblint_check(xs == 3); // TO-DO y == [6,8]; this does not work because
                              // CIL transforms this into two if-statements
  }
}
