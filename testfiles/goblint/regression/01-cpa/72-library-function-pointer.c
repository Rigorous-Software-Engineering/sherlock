...
// Goblint used to crash on this example because the number of supplied
// arguments for myopen is bigger than than the expected number of arguments for
// the library function descriptior of ftruncate.
...
int main() {
  ...
  if (top2) {
    ...
    // So no warning is emitted here about possibly calling the vararg function
    // open.
    myftruncate2(0, v);
  } else {
    // Warn about potential calls to open and ftruncate with too few arguments,
    // and warn that none of the possible targets of the pointer fit as call
    // targets.
    ptr(); // WARN
  }
  return 0;
