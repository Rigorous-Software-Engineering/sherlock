...
// There was an issue when testing arrays, as they init to bottom and this code
// triggers narrowing.
...
struct sockaddr_in {
  ...
  unsigned char sin_zero[((sizeof(struct sockaddr) - sizeof(unsigned short)) -
                          sizeof(in_port_t)) -
...
int main(int argc, char **argv) {
  ...
  {
    while (1) { // This can cause leq issues as it triggers narrowing in struct
                // - arrays on uninitialized hist_data!
      if (!error) {
        if (!(c != -1)) {
          break;
        }
