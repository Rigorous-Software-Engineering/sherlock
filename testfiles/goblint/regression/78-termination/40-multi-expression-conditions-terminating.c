...
int main() {
  ...
  for (i = 1; i <= 10; i++) {
    if (i > 5 &&
        i % 2 == 0) // CIL defines new jump labels to default location (-1)
    {
      printf("%d ", i);
    }
  }
  ...
  int s = 1;
  while (s <= 10 &&
         s % 2 == 0) // CIL defines new jump labels to default location (-1)
  {
  ...
  int t, u;
  for (t = 1, u = 10; t <= 5 && u >= 5;
       t++, u--) // CIL defines new jump labels to default location (-1)
  {
