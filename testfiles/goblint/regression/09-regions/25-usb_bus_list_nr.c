// SKIP PARAM: --set ana.activated[+] "'region'"  --set kernel true --set
...
static struct dev *find_dev(int id) {
  ...
  for (buslist = bus_list.next; buslist != &bus_list; buslist = buslist->next) {
...
static void add_bus(struct dev *d) {
  struct usb_bus *bus;
