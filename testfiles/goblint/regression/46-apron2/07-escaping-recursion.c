...
int rec(int i, int *ptr) {
  ...
  if (i == 0) {
    ...
    // If we analyse this with int contexts, there is no other x that is
    // reachable, so this update is strong
    x = 17;
    ...
    *ptr = 12;
    ...
    // Another copy of x is reachable, so we are conservative and do a weak
    // update
    x = 31;
