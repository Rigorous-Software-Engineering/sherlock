...
/* Example from "Towards an industrial use of FLUCTUAT on safety-critical
   ...
   Sine using an interpolation table.
*/
...
const double T[362] = {0.00000000000000,
                       0.01745240643728,
                       0.03489949670250,
                       0.05233595624294,
                       0.06975647374413,
...
double SIN_POS(double x) {
  if (x > 180) {
    return SIN_180(x - 360. * (int)((x + 180.) / 360.));
  } else {
    return SIN_180(x);
  }
}
...
int main() {
  ...
  for (a = -360; a <= 360; a++) {
    ...
    __VERIFIER_assume(aa >= a / 2. && aa <= (a + 1.) / 2.);

    r = SIN(aa) + SIN(-aa);

    __VERIFIER_assert(r >= -0.06 && r <= 0.06);
  }
  return 0;
}
